...
void ComponentCharacterController::Update(float p_fDelta)
{
	...
	if ((m_bKeysDown['w'] || m_bKeysDown['W']))
	{
		if (!m_bWalking)
		{
			...
			{
				m_fWalkingDelegate(5 /* Number is just to demonstrate using std::tr1::function with params */);
			}
		}
		m_bWalking = true;
	}
	else
	{
		if (m_bWalking)
		{
			...
			{
				m_fStopWalkingDelegate(2 /* Number is just to demonstrate using std::tr1::function with params */);
			}
		}
		m_bWalking = false;
	}
